...
# coding=utf-8
"""
Stage 1 trainer v2: **Llama + IP2P channel concat with PRE-CACHED VAE
latents** for ~30-40% wall-clock speedup vs the v1 trainer.

Identical to train_editclip_emmdit_ip2p.py EXCEPT:
  * No VAE is loaded or invoked during training.
  * The DataLoader reads pre-encoded latents from disk (one .pt per sample)
    produced by precompute_vae_cache.py.
  * `transforms.RandomCrop` is gone (cached latents are pre-resized).
    `RandomHorizontalFlip` still applied on latents.
  * --dataset_name is replaced by --vae_cache_dir.

Cache layout (produced by precompute_vae_cache.py)
--------------------------------------------------
    <vae_cache_dir>/metadata.json
    <vae_cache_dir>/00000000.pt   {"orig": tensor, "edit": tensor, "text": str}
    <vae_cache_dir>/00000001.pt
    ...

Each tensor is the already-VAE-scaled latent (i.e. encoded * scaling_factor),
shape [32, H/32, W/32] in the dtype chosen at cache time (bf16 default).

Speedup
~~~~~~~
v1: 1.02 it/s on 1x H200 (incl. 2x VAE encode per step).
v2: ~1.7-1.8 it/s expected (VAE skipped entirely).

Stage 1 trainer (original / v1 docstring follows for reference)
--------------------------------------------------------------
...
class IP2PCachedDataset(torch.utils.data.Dataset):
    """Dataset over pre-encoded latents produced by precompute_vae_cache.py.

    Returns:
      - original_latent   [32, H/32, W/32]  (VAE-scaled, ready to use)
      - edited_latent     [32, H/32, W/32]
      - text              str   (edit instruction)

    `RandomHorizontalFlip` is applied on the latents (paired so they stay
    aligned). `RandomCrop` is NOT available — input images were pre-resized
    to the target resolution before caching.
    """
    def __init__(self, cache_dir: str, hflip_prob: float = 0.5):
        self.cache_dir = Path(cache_dir)
        if not self.cache_dir.is_dir():
            raise FileNotFoundError(
                f"cache_dir does not exist: {cache_dir}. "
                f"Run precompute_vae_cache.py first."
            )

        # Scan all <idx:08d>.pt files in the cache.
        files = sorted(self.cache_dir.glob("[0-9]" * 8 + ".pt"))
        if not files:
            raise RuntimeError(
    ...
    def __getitem__(self, idx):
        data = torch.load(self.files[idx], map_location="cpu",
                           weights_only=False)
        orig = data["orig"]  # [32, H/32, W/32]
        edit = data["edit"]
        text = data.get("text", "") or ""

        # Paired horizontal flip on latents (last dim = width).
        if self.hflip_prob > 0 and torch.rand(1).item() < self.hflip_prob:
            orig = torch.flip(orig, dims=[-1])
            edit = torch.flip(edit, dims=[-1])

        return {
            "original_latent": orig.contiguous(),
            "edited_latent":   edit.contiguous(),
            "text":            str(text),
        }
...
def collate_fn(examples):
    return {
        "original_latent": torch.stack(
            [e["original_latent"] for e in examples]
        ).contiguous(),
        "edited_latent": torch.stack(
            [e["edited_latent"] for e in examples]
        ).contiguous(),
...
def parse_args():
    ...
    # Dataset — uses PRE-CACHED VAE latents (see precompute_vae_cache.py)
    p.add_argument("--vae_cache_dir", type=str, required=True,
                   default=None,
                   help="Path to directory of cached latent .pt files. "
                        "Run precompute_vae_cache.py first.")
    p.add_argument("--max_train_samples", type=int, default=None)
    p.add_argument("--dataloader_num_workers", type=int, default=4)
    p.add_argument("--hflip_prob", type=float, default=0.5,
                   help="Probability of paired horizontal flip on latents.")
    # Training
...
def main():
    ...
    # ------------------------------------------------------------------
    # 2. Llama text encoder  (FROZEN, offline-only)
    # ------------------------------------------------------------------
    ...
    # VAE is NOT loaded in v2 — latents come pre-encoded from cache.
    logger.info("[v2] skipping VAE load — using cached latents from "
                f"{args.vae_cache_dir}")
    ...
    # ------------------------------------------------------------------
    # 6. Dataset / DataLoader  (cached latents — no VAE encoding here)
    # ------------------------------------------------------------------
    logger.info(f"[v2] using cached latent dataset: {args.vae_cache_dir}")
    train_dataset = IP2PCachedDataset(
        cache_dir=args.vae_cache_dir,
        hflip_prob=args.hflip_prob,
    )
    logger.info(f"[v2] dataset size = {len(train_dataset)} cached samples")
    if train_dataset.metadata:
        md = train_dataset.metadata
        logger.info(f"[v2] cache metadata: resolution={md.get('resolution')}, "
                    f"latent_shape={md.get('latent_shape')}, "
                    f"vae_scaling_factor={md.get('vae_scaling_factor')}, "
                    f"scaled_latents={md.get('scaled_latents')}")
        if (md.get("resolution") is not None
                and md["resolution"] != args.resolution):
            raise RuntimeError(
                f"Cache resolution ({md['resolution']}) != args.resolution "
                f"({args.resolution}). Re-run precompute_vae_cache.py at "
                f"the desired resolution."
            )
    if args.max_train_samples is not None:
        # Truncate by sub-sampling the file list deterministically.
        train_dataset.files = train_dataset.files[: args.max_train_samples]
        logger.info(f"[v2] truncated to first {args.max_train_samples} samples")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                # Latents already VAE-encoded + scaled by precompute_vae_cache.py.
                edited_latents = batch["edited_latent"].to(
                    accelerator.device, dtype=weight_dtype, non_blocking=True
                )
                if not args.no_channel_concat:
                    original_latents = batch["original_latent"].to(
                        accelerator.device, dtype=weight_dtype, non_blocking=True
                    )
